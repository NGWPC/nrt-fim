...
from trainer.datasets.registry import compute_in_channels, VAR_NAME_MAP
...
def pad_for_model(
    ...
    y: Optional[torch.Tensor]=None, # optional target, (1,H,W) or (N,1,H,W)
    multiple: int = 32,
    min_size: Optional[Tuple[int,int]] = None,  # (min_h, min_w). If None, skip this step.
):
    """
...
def forward_full_image_tiled(
    ...
    aoi_bbox: Optional[Tuple[float, float, float, float]] = None,  # (minx,miny,maxx,maxy) in master-grid CRS
                         # override; else taken from ds.flood_instances[idx]["end_time"]
    master_path: Optional[str] = None,  # override; else ds.master_path
):
    """
